...
export const buildSwapContract = (
  secretHash,
  publicKeyString,
  timelock,
...
export const buildSpendScript = (
  ...
  spendingAtomicSwapScript.addData(signature).addData(publicKeyString);
  if (refund) {
    spendingAtomicSwapScript.addOp(Opcodes.OpFalse);
  } else {
    spendingAtomicSwapScript.addData(secret).addOp(Opcodes.OpTrue);
  }
