import {
  ...
  signMessage,
  verifyMessage,
} from "../kaspa-wasm32-sdk/web/kaspa/kaspa";
...
export const getPricesRequest = (swapType, privKey, pubkey) => {
  ...
  const requestBody = getGeneralRequest(
    MessageType.PRICE,
    payload,
    privKey,
    pubkey,
...
export const getSat2kasInitSwapRequest = (
  receiverAddress,
  amount,
  price,
  privKey,
...
export const getDataFromMaker = async (requestBody, makerAddress) => {
  ...
  try {
    ...
    const isMakerSignatureValid = verifyMessage({
      message: message,
      signature: signature,
      publicKey: publicKey,
    });
